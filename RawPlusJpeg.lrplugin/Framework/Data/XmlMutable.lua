...
function Attributes:serialize( ind )
    local b = {}
    for i, v in ipairs( self.array ) do -- this is a bug, since attr values changed in lookup table aren't reflected in array - fixed in XmlBuffer, another reason this class is deprecated.
        b[#b + 1] = "\n" .. ind .. v.name .. '="' .. v.text .. '"'
    end
    if #b > 1 then
...
function Element:serialize( initialIndentation, spacesPerIndent )
    local function _serialize( node, indent, spaces ) -- serialize element node
        ...
        if str:is( t ) and #node == 0 then
            s = s .. '>' .. t .. '</' .. name .. ">\n"
        else -- note: although this creates legal xml, it won't work for gradients in xmp, since the "short form" is required for empty elements. - use XmlBuffer instead.
            s = s .. '>\n'
